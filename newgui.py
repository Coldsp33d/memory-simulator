...
class Mem():
	def __init__(self,master,choice):
		...
		#self.e1.config(indicatoron=0,compound="right",image=self.arrow,width=140)
...
class OldDialogDemo(Frame):
	...
	def compile_method(self):
		...
		if MU0gen.syn_error==False:
			...
			#tkinter.messagebox.showwarning('ERROR!', MU0gen.err)
			self.cont.config(state="disabled")
			self.errvar.set(MU0gen.err)
			MU0gen.err=''
			MU0gen.syn_error=False
			tru=False
			#if tkinter.messagebox.askyesno('Question', 'Do you want to clear the screen?'):
	...
	def help(self):
		...
		#root.option_add("*Dialog.msg.wrapLength", "12i")

		#tkinter.messagebox.showinfo("Final",s)
	def dele(self):
		self.errvar.set('')
		self.text.delete("1.0", END)
		self.cont.config(state="disabled")
